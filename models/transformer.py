...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        return x + self.pe[:, :x.size(1),:]     # x: [batch_size,seq_len, d_model]
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        """
        scores = torch.matmul(Q, K.transpose(-1, -2)) / np.sqrt(Q.shape[-1])  # scores : [batch_size, n_heads, len_q, len_k]
        # mask矩阵填充scores（用-1e9填充scores中与attn_mask中值为1位置相对应的元素）
        # Fills elements of self tensor with value where mask is True.
        scores.masked_fill_(attn_mask, -1e9)   #为True的地方用最小值填充
...
class EncoderLayer(nn.Module):
    def __init__(self,d_model,d_ff,d_k,d_v,n_heads,device):
        ...
        self.enc_self_attn = MultiHeadAttention(d_model,d_k,d_v,n_heads,device)
...
class Encoder(nn.Module):
    def __init__(self,src_vocab_size,max_len,d_model,d_ff,d_k,d_v,n_heads,device):
        super(Encoder, self).__init__()
        self.src_emb = nn.Embedding(src_vocab_size, d_model)  # token Embedding
        self.pos_emb = PositionalEncoding(d_model, max_len)  # Transformer中位置编码时固定的，不需要学习
    ...
    def forward(self, enc_inputs):
        ...
        for layer in self.layers:  # for循环访问nn.ModuleList对象
            ...
            enc_outputs= layer(enc_outputs, enc_self_attn_mask)  # 传入的enc_outputs其实是input，传入mask矩阵是因为你要做self attention

        return enc_outputs


class Decoder(nn.Module):
    def __init__(self,tgt_vocab_size,max_len,d_model,d_ff,d_k,d_v,n_heads,device):
        super(Decoder, self).__init__()
        self.tgt_emb = nn.Embedding(tgt_vocab_size, d_model)  # Decoder输入的embed词表
        self.pos_emb = PositionalEncoding(d_model, max_len)
        self.layers = nn.ModuleList([DecoderLayer(d_model,d_ff,d_k,d_v,n_heads,device) for _ in range(n_layers)])  # Decoder的blocks
        self.device = device
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        dec_self_attn_mask = torch.gt((dec_self_attn_pad_mask + dec_self_attn_subsequence_mask),0).to(self.device)  # [batch_size, tgt_len, tgt_len]; torch.gt比较两个矩阵的元素，大于则返回1，否则返回0
...
class Transformer(nn.Module):
    def __init__(self,src_vocab_size,tgt_vocab_size,max_len,d_model,d_ff,d_k,d_v,n_heads,device):
        super(Transformer, self).__init__()
        self.encoder = Encoder(src_vocab_size,max_len,d_model,d_ff,d_k,d_v,n_heads,device).to(device)
        self.decoder = Decoder(tgt_vocab_size,max_len,d_model,d_ff,d_k,d_v,n_heads,device).to(device)
...
if __name__=="__main__":
    ...
    # 调试的时候batchsize设置成1比较好
    dataset=MyDataSet(enc_inputs, dec_inputs, dec_outputs)
    loader = Data.DataLoader(dataset, 3, True)

    model = Transformer(src_vocab_size,tgt_vocab_size,max_len,d_model,d_ff,d_k,d_v,n_heads,device).to(device)
    ...
    enc_inputs, dec_inputs, dec_outputs = make_data(sentences,src_vocab,tgt_vocab)
    test_loader = Data.DataLoader(MyDataSet(enc_inputs, dec_inputs, dec_outputs), 1, True)
